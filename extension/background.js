/**
 * Routes a fill intent from an EdgeDesk tab to the exchange tab. Focuses
 * the first open Betdaq tab; if none is open we do nothing (the stake is
 * already on the clipboard - EdgeDesk said so in its toast).
 */
chrome.runtime.onMessage.addListener((message, _sender) => {
  if (message?.type !== "edgedesk-fill-slip") return;
