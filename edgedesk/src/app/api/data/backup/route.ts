  ...
  if (format === "json") {
    ...
    return new NextResponse(JSON.stringify(bundle), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="edgedesk-backup-${stamp()}.json"`,
      },
    });
  }

  // WAL-safe online snapshot via better-sqlite3's backup API.
  const tmp = path.join(os.tmpdir(), `edgedesk-backup-${Date.now()}.db`);
  try {
    ...
    return new NextResponse(new Uint8Array(bytes), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="edgedesk-backup-${stamp()}.db"`,
      },
    });
  } finally {
