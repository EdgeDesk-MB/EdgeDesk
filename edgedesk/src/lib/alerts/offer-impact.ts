...
export function offerExpiringAlertCopy(args: {
  ...
  const evExact = args.remainingEv.toFixed(2);
  ...
  if (args.impact.source !== "expiry" && until > 0) {
    ...
    return {
      title: `${where} off in ${mins} min - £${pounds} unclaimed`,
      body: `${args.offerTitle}: £${evExact} of edge - place the qualifying bet before the off.`,
    };
  }
  ...
  if (args.impact.source !== "expiry" && until <= 0) {
    const where = place ?? "Meeting";
    return {
      title: `${where} is under way - £${pounds} unclaimed`,
      body: `${args.offerTitle}: £${evExact} of edge still open - finish the qualifying bet while the scope applies.`,
    };
  }
  ...
  if (until > 0 && until <= OFFER_EXPIRY_LEAD_MS) {
    ...
    const when =
      mins >= 60
        ...
        : `${Math.max(1, mins)} min`;
    return {
      title: `Offer ends in ${when} - £${pounds} unclaimed`,
      body: `${args.offerTitle}: £${evExact} of edge expires at ${formatClockTime(args.impact.at)}.`,
    };
  }

  return {
    title: `Offer ends today - £${pounds} unclaimed`,
    body: `${args.offerTitle}: £${evExact} of edge expires with it.`,
  };
}
