...
export function CasinoComponentForm({
  ...
  const [wageringMultiplier, setWageringMultiplier] = useState(
    existing?.wageringMultiplier ?? initialValues?.wageringMultiplier ?? (existing ? NaN : 35)
  );
  ...
  function buildPayload(): Record<string, unknown> {
    ...
    switch (componentType) {
      case "qualifying_wager":
        return { componentType, amount, rtp };
      case "cash":
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {componentType === "bonus" || componentType === "free_spins" ? (
        <CasinoGamePicker
          games={games}
