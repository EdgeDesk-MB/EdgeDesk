...
Last updated: 2026-09-14 (P2b Bookie scoping brief under EDGE-148. Canonical
home is Accounts. Early-payout Desk focuses the Early-payout surface.
Production remains the live app: `SITE_SURFACE=app`, `LANDING_VARIANT=launch`
on https://edgeways.app.)
...
| D5b | **Renamed EdgeDesk → Edgeways** (lowercase "edgeways" wordmark). Chamfered-bolt placeholder mark; push titles lead with ⚡. "Offer Edge" tier + `--edge` violet unchanged.                                        | 2026-08-04 | Trademark clash forced a clean break from the EdgeDesk mark. "Edgeways" keeps the Edge language, owns the British idiom form, and the bolt gives notifications a native brand mark. Domain: edgeways.app (Sam handles). GitHub org/repo rename still outstanding, tracked as EDGE-143. **Renumbered from D7 on 2026-09-13:** this decision and operator-held feeds were both carrying the label D7, and `docs/decisions/d7-operator-held-feeds.md` owns that number. The rename has no decision file of its own. |
| D7  | **Operator-held feeds. The customer brings money, Sam brings every provider account.** Subscribers never buy, paste or manage Racing API, API-Football or Betfair keys. BYOK is rejected. Edgeways holds the keys, polls once, caches, and fans out to entitled desks. | 2026-08-22 | Collapses API cost from "every user buys keys" to £2–6/mo COGS at 50–100 subscribers, and it is the product rather than a later optimisation. One match watched serves all watchers. Full brief: `../../../docs/decisions/d7-operator-held-feeds.md`. Pooled feeds are EDGE-81; server-side entitlement guards are EDGE-83. |
| D8  | **Staged licensing: comply-on-contact.** Launch on operator-held pooled feeds **without** written redistribution permission, and resolve licensing when approached, from reserved revenue. No permission emails sent in advance.                                     | 2026-08-23 | Amends D7's original ask-first framing for the launch stage. Asking first converts later use from naive to knowing breach and puts us on the radar for nothing. Reply templates are drafted and deliberately unsent (`../../../docs/legal/provider-permission-emails.md`). Betfair stays delayed-only. Customer-facing copy never names a provider. Full brief: `../../../docs/decisions/d8-staged-licensing.md`. |
| D9  | **Multi-sport feed strategy.** Racing and football are the launch sports. Further sports are added in UK bookmaker offer-volume order (**tennis, golf, greyhounds, then NFL / darts / snooker / cricket**), funded by Edge revenue, each shipping with a cap and a monitor from day one. | 2026-08-26 | "All sports" is a direction, not a launch requirement. Phase 1 (tennis + golf via the Betfair catalogue we already integrate, £0 incremental) is EDGE-88. Full brief: `../../../docs/decisions/d9-multi-sport-feed-strategy.md`. |
...
| **17. Each-way & Extra-place** (L7, unparked from §9 on 2026-08-06)                                     | ✅ Phases 1–4: calculator dialog handoff from Racing Desk, UK place ladder + min-runners, Active bets strip + PlaceZoneBar, dual-lay settle tests. Betfair PLACE feed still stretch; multi-dutch later                                                                                                                      | **Done** (2026-08-06)                                          | The EW / Extra-place calculator is the place of truth with dual win+place lays; Racing Desk opens it prefilled; saves land in Profit Tracker with `serializeEwMeta` and auto-settle through `settleDualLayEachWay`; open racing positions read as a live strip, not a dead count                                                                                                                                              |
| **18. Offer completion playbook** (O1)                                                                  | ✅ Phase 1: deposit codes through to clearing wagering, best-effort deposit auto-complete from Accounts transfer/top-up, convert prefills carrying reward labels and date-lock notes, clear-wagering WR watch                                                                                                              | **Done** (2026-08-10)                                          | A campaign carries its completion steps; a deposit logged on Accounts ticks the matching playbook step without re-entry; wagering requirements are watched and surfaced rather than remembered                                                                                                                                                                                                                              |
| **19. Football live card** (P1)                                                                         | ✅ Phase 1 (EDGE-128): match tape (goals, cards, VAR, subs), XI + formations, goals into History as they happen, HT/ET/pens clocks, 15s live poll, store-first day cards, named-player goalscorer picks. **Phase 1b** HT/FT auto-settle (`/calc-change`, EDGE-141), **Phase 2**, **Phase 3** outstanding                    | **Phase 1 done** (2026-09-11)                                  | An Edge football match carries its tape and XI the way Racing Desk carries a racecard, so settlement and Add bet stop being free text; one shared poll per `externalId`; Core keeps live score; customer copy never names the provider (D8)                                                                                                                                                                                 |
| **20. Early-payout Desk as a live desk** (P2)                                                           | Rename chrome to Early-payout Desk (`/early-payout`; `/2up` permanently redirects). Board-first like Racing Desk. Football 2UP is the modelled form (tape, picks, Dixon-Coles). Bookie **scoping** lives on Accounts (Early payout now, Racing later). The desk focuses Early-payout scope only. Active lists every early-payout bet; other sports stay on Tracker until a model exists. Fixture horizon past tomorrow stays later. | **In progress** (EDGE-148 + P2b children)                      | An early-payout regular can live on one desk: today's football, Edge 2UP picks, scoped books, tracked fixtures, Active across sports, then the football model when they pick a match. Adding an early-payout bet can confirm a new bookie scope.                                                                                                                                                                                                                         |

> **Sequencing hygiene note (2026-09-13).** Phases 17–19 were added during an
> organisation audit. L7, O1 and P1 all had full implementation briefs and had
> shipped, but none had a row here, which broke the roadmap-hygiene rule at the
> top of this document. Work is now tracked in Linear under the **Post-Launch
> Desk & Live Sport** project as well as here.
...
| iOS PWA push reliability in practice (backgrounded Safari)     | Still open. F3 web push verified on Sam's Android 2026-07-14; iOS has never been tested on real hardware. Fallback remains timeline-visible alerts + email |
| Racing API rate limits vs multi-race live polling on race days | Largely answered by operator-held pooled feeds (D7) plus per-feed daily caps that fail closed (EDGE-100, EDGE-89). Football live poll moved to 15s on the paid quota 2026-09-11. Watch as subscriber count grows |
...
| **Each-way matcher (Racing Desk)**               | EW / Extra place: desk → calculator modal (dual lays) → tracker → race-result settle; active bets + place-zone visual                                                                                                                                                                                                                                                                                          | **PROMOTED → L7 / Phase 17 (2026-08-06). ✅ DONE same day** (Phases 1–4). Not OM discovery EV. Betfair PLACE feed still stretch; multi-dutch later. See `implementation-briefs.md` L7.                                                                                                                                  |
...
| **Referral codes (unique, Stripe)**              | Friend-gets-discount / you-get-credit is the cheapest growth loop before paid ads. Stripe promotion codes + Customer Balance.                                                                                                                                                                                                                 | **PROMOTED → EDGE-67 (2026-08-16).** Pre-live. Not a 40% affiliate programme. See `docs/strategy/oddsmonkey-onboarding-research.md`.                                                                                                                                                                                   |
| **Import from Oddsmonkey / Outplayed CSV**       | Finder users will not retype years of P&L. Dedicated profits-CSV adapters, progress bar, empty-Home CTA with their logos.                                                                                                                                                                                                                     | **PROMOTED → EDGE-68 (2026-08-16).** Oddsmonkey sample in `docs/strategy/fixtures/`. Enum dump still needed. Outplayed waits on a sample. See `docs/strategy/platform-import.md`.                                                                                                                                     |
| **Affiliate programme (recurring % payouts)**    | Oddsmonkey-style 40% of subscription revenue, dashboard, assets. Needs payouts, tax, abuse handling.                                                                                                                                                                                                                                          | **Parked (2026-08-16).** After Edge revenue. Referral *codes* ship first.                                                                                                                                                                                                                                             |
