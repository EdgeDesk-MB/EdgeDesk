...
| **Acca desk (leg-by-leg lay workflow)** | Acca offers run over days; track each leg, compute the lay-on-loss stake as legs settle, alert "lay leg 3 now". Outplayed's Acca Catcher proves demand — but their tracker needs their finder; ours works from any acca the user logs. Pure execution workflow, zero discovery | NEW from 2026-07-16 landscape research (see competitive-landscape.md) — awaiting Sam's review |
| **Mug-bet scheduler** | Plan and budget camouflage bets per bookie (cadence, spend as a deliberate cost against that account's EV earned); feeds B9 health. Nobody does this systematically | NEW 2026-07-16 — awaiting Sam's review |
| **Measured effort — real £/hr** | Time the actual open→logged span per offer so B2's £/hr uses measured minutes, not estimates. The purest execution-truth feature; no competitor measures execution | NEW 2026-07-16 — awaiting Sam's review |
| **Casino variance simulator** | Local Monte Carlo over a wagering offer: bust probability, median outcome, distribution chart — extends H2's variance tiers into honest distributions. Incumbents publish static EV lists only | NEW 2026-07-16 — awaiting Sam's review. Calc-heavy: needs spec + test vectors first |
| **Lock-in advisor (any open position)** | Generalise the 2UP equalise maths: any open back+lay shows "close now for £X guaranteed" from user-entered current odds | NEW 2026-07-16 — awaiting Sam's review |
| **Price boost & bet builder checkers** | Manual-entry fair-price verdicts (extends F1 Match Checker): log a boost or builder price vs exchange fair odds → EV verdict + a "boost diary" of EV captured. The incumbents' finders are D2; the checkers are not | NEW 2026-07-16 — awaiting Sam's review |
| **Offer email ingestion** | Forward/drag bookie promo emails into the desk; the existing parse pipeline prefills the offer. Kills the daily-calendar gap without any scraping | NEW 2026-07-16 — awaiting Sam's review. Needs an ingestion design (IMAP pull vs .eml drop) |
| **Betslip prefill extension** | Betwatch-style browser extension: one click in EdgeDesk fills the exchange/bookie slip with selection + stake. Execution automation, not discovery | NEW 2026-07-16 — awaiting Sam's review. New surface (extension) — size carefully |
| **Household account sets** | Track a partner's separately-operated accounts as a second set (owner tag on accounts, per-owner P&L). Long-standing community practice the incumbents support | NEW 2026-07-16 — awaiting Sam's review. Compliance-sensitive framing required |
