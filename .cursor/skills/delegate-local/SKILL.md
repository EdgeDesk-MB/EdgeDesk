...
The main agent acts as foreman: writes a self-contained prompt, ships it to the local node, reviews the result like a PR from a junior. Run this from a frontier model — the point is frontier oversight of local output; a local-quality model reviewing its own draft defeats it.
...
1. Build a fully self-contained prompt. The local model has NO repo access: inline every file excerpt, data shape and acceptance criterion it needs. Keep the prompt under roughly 20K tokens; practical local context is 32–64K.
