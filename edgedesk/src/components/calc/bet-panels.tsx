...
interface PanelProps {
  ...
  className?: string;
  /** Base colour; defaults to MBB green (back) / blue (lay) */
  color?: string;
...
function Panel({ title, children, className, color, chip }: PanelProps) {
  ...
  return (
    ...
    >
      <div className="bg-[var(--panel)] p-4 dark:bg-[var(--panel-dark)]">
...
export function LayPanel({
  exchange,
  ...props
}: Omit<PanelProps, "color"> & { exchange?: ExchangeRow | null }) {
  return <Panel {...props} color={exchange?.layColor ?? LAY_LIGHT} />;
}
  ...
  return (
    <label className={cn("flex flex-col gap-1", className)}>
      ...
      <input
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="h-11 w-full rounded-md border-0 bg-[var(--pi)] px-3 text-base font-semibold text-black/85 outline-none ring-primary/40 placeholder:text-base placeholder:font-medium placeholder:text-black/40 focus:ring-2 dark:bg-[var(--pi-dark)] dark:text-white/95 dark:placeholder:text-white/40"
      />
    </label>
  );
}
              ...
              return (
                <tr key={row.label} className="border-t">
                  <td className="py-2 pr-3">
                    <span
                      className="flex min-h-9 items-center bg-[var(--chev)] py-1 pl-3 pr-6 text-[13px] font-bold leading-tight text-[var(--chev-text)] dark:bg-[var(--chev-dark)] dark:text-white/95"
                      style={
                        {
                          "--chev": accent,
