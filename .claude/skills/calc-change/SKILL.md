---
name: calc-change
description: Guarded workflow for any change to EdgeDesk calc or settlement logic (edgedesk/src/lib/calc, edgedesk/src/lib/offers). Use BEFORE editing stake, lay, commission, EV, or settlement maths, or when a task mentions odds, free bets, extra places, Rule 4, dutching, or profit calculations.
---
...
## Steps
1. Read the relevant section of `edgedesk/docs/offer-command-centre.md` and the
...
4. From `edgedesk/`, run `npx vitest run`. The full suite must stay green.
   NEVER weaken or delete a passing test to get there.
5. If the change affects any EV figure shown in the UI, respect the EV
   provenance rule in `edgedesk/docs/roadmap/`: displayed EV must state its basis.
