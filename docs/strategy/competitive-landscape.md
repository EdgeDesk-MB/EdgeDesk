...
|---|---|---|
| OddsMatcher (real-time back/lay across ~90 bookies) | OddsMonkey, Outplayed, BonusMoney | **D2: no oddsmatching, possibly never** — it is *discovery*, their core cost, and their moat. Edgeways sells *execution truth*. Unchanged. |
...
which is exactly Edgeways' thesis. The gaps that fall out of this research:
...
here — confirm on the provider dashboards before treating any figure as current). Edgeways status
...
| Feature | OddsMonkey | Outplayed | Edgeways | Notes |
|---|---|---|---|---|
...
sequenced in Phase 11 (J1–J9). The ✅ rows are where Edgeways already matches or exceeds incumbent
...
automation" rows in §4 being ✅ for Edgeways and thin-to-absent for both incumbents. Working
line: **"They tell you what to bet. Edgeways tells you whether it actually paid."**

This is deliberately not "Edgeways does everything OddsMonkey does, cheaper." It doesn't — see the
...
subscription (£49.99, either provider) *plus* Edgeways for everything execution-side — because
Edgeways was never trying to replace the part of the bundle it deliberately doesn't build (D2).
Pricing Edge against the £150-300 bundle would mean competing on a feature set Edgeways doesn't
...
Ultra/Elite outright, and Edgeways' slice of that is cheap because it carries no discovery feed
cost (§7.2 — feed proxy economics only apply once multi-tenant; today it's £0-40/mo in personal
API costs per `docs/strategy/api-dependencies-and-tiers.md`).
...
a single user runs Edgeways, and none of it is replicable by an incumbent bolting on a tracker
