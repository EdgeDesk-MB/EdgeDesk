...
/**
 * Boost checker (J2) in the Add bet idiom: stacked Back/Lay panels, the
...
export const BOOSTS_CHANGED_EVENT = "edgedesk:boosts-changed";
...
export function BoostCheckerForm({
  ...
  useEffect(() => {
    if (exchange || exchanges.length === 0) return;
    const preferred = exchanges.find((e) => e.isDefault) ?? exchanges[0];
  ...
  // Lay plan (boost mode only - a same-match builder can't be laid as one bet)
