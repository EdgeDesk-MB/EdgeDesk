...
- **Calculator index cards** - `/calculators` hub cards use the same leading
  ...
  (association football: circle, pentagon, five panels). Racing Desk uses
  `HorseRacingIcon` (Lucide Lab horse-head) in nav, empties and welcome, not
  Trophy. Both marks live in `sport-icon.tsx`.
